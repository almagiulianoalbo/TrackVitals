...
export async function POST(request: NextRequest) {
  ...
  if (!isUserRole(body.role)) {
    return jsonError("Seleccioná si ingresás como paciente o médico.");
  }
  ...
  if (!email || !password) {
    return jsonError("Ingresá email y contraseña.");
  }
  ...
  try {
    ...
    if (body.role === "medico") {
      ...
      if (!data || !verifyPassword(password, data.password_med)) {
        return jsonError("Email, contraseña o tipo de usuario incorrectos.", 401);
      }
    ...
    console.error(error);
    return jsonError("No se pudo iniciar sesión. Revisá la configuración del servidor.", 500);
  }
}
