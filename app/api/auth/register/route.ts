...
export async function POST(request: NextRequest) {
  ...
  if (!isUserRole(body.role)) {
    return jsonError("Seleccioná si estás registrándote como paciente o médico.");
  }
  ...
  if (!nombre || !apellido || !email || !password) {
    return jsonError("Completá nombre, apellido, email y contraseña.");
  }

  if (!isValidEmail(email)) {
    return jsonError("Ingresá un email válido.");
  }
  ...
  try {
    ...
    if (body.role === "medico") {
      ...
      if (!matricula) {
        return jsonError("Ingresá la matrícula profesional.");
      }
      ...
      if (!data) {
        return jsonError("Supabase no devolvió los datos del médico creado.", 500);
      }
    ...
    if (!dni) {
      return jsonError("Ingresá el DNI del paciente.");
    }

    if (idMedicoCabecera === false) {
      return jsonError("El ID del médico de cabecera debe ser numérico.");
    }
    ...
    if (!data) {
      return jsonError("Supabase no devolvió los datos del paciente creado.", 500);
    }
    ...
    console.error(error);
    return jsonError("No se pudo completar el registro. Revisá la configuración del servidor.", 500);
  }
}
...
async function getNextId(supabase: SupabaseAdmin, table: "medicos" | "pacientes", idColumn: "id_medico" | "id_paciente") {
  ...
  if (!Number.isSafeInteger(value) || value < 1) {
    return {
      error: {
        message: `No se pudo calcular el próximo valor de ${idColumn}.`
      }
    };
  }
...
function handleSupabaseInsertError(error: SupabaseInsertError) {
  ...
  if (error.code === "23505" || text.includes("duplicate") || text.includes("unique")) {
    ...
    if (text.includes("matricula")) {
      return jsonError("Ya existe un médico con esa matrícula.", 409);
    }
    ...
    if (text.includes("id_medico") || text.includes("medicos_pkey")) {
      return jsonError("No se pudo generar un ID nuevo para el médico. Revisá la primary key id_medico.", 500);
    }

    if (text.includes("nombre")) {
      return jsonError("Supabase está rechazando el nombre por una restricción UNIQUE. Revisá los constraints de la tabla.", 500);
    }

    return jsonError("Ya existe un registro con una restricción única de Supabase.", 409);
  }

  if (error.code === "23502") {
    if (text.includes("id_paciente")) {
      return jsonError("La tabla pacientes no está generando id_paciente automáticamente.", 500);
    }

    if (text.includes("id_medico")) {
      return jsonError("La tabla médicos no está generando id_medico automáticamente.", 500);
    }
  ...
  if (error.code === "23503" || text.includes("foreign key")) {
    return jsonError("No se encontró el médico de cabecera indicado.");
  }

  return jsonError("No se pudo crear la cuenta. Verificá los datos ingresados.", 500);
}
